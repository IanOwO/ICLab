...
//           gate-or                            //
//==============================================//
wire max_pool_map_clk;
wire quant2_clk;
wire feature_clk;
wire weight_clk;
wire ker_clk;

wire flag_max_pool_clk;

GATED_OR GATED_weight(.CLOCK(clk),.SLEEP_CTRL((counter > 4) & cg_en),.RST_N(rst_n),.CLOCK_GATED(weight_clk));
GATED_OR GATED_ker(.CLOCK(clk),.SLEEP_CTRL((counter > 9) & cg_en),.RST_N(rst_n),.CLOCK_GATED(ker_clk));
GATED_OR GATED_max_pool_map(.CLOCK(clk),.SLEEP_CTRL(!(max_pool_flag) & cg_en),.RST_N(rst_n),.CLOCK_GATED(max_pool_map_clk));
GATED_OR GATED_quant2(.CLOCK(clk),.SLEEP_CTRL(!(counter == 25 | counter == 37 | flag_swap | flag_dist) & cg_en),.RST_N(rst_n),.CLOCK_GATED(quant2_clk));
GATED_OR GATED_feature(.CLOCK(clk),.SLEEP_CTRL((!feature_flag) & (!feature_pass) & cg_en),.RST_N(rst_n),.CLOCK_GATED(feature_clk));

GATED_OR GATED_flag_max_pool(.CLOCK(clk),.SLEEP_CTRL(!((counter == 22) | out_valid) & cg_en),.RST_N(rst_n),.CLOCK_GATED(flag_max_pool_clk));
...
always @(posedge flag_max_pool_clk or negedge rst_n) begin
	if(!rst_n) begin
		flag_max_pool <= 1'b0;
	end
	else if(out_valid) begin
...
// input
// ======================================
always @(posedge weight_clk) begin
	if(in_valid) begin
		if(counter < 4) begin
...
always @(posedge feature_clk) begin
...
always @(posedge feature_clk or negedge rst_n) begin
	if(!rst_n) begin
		feature_map[0] <= 0;
		feature_map[1] <= 0;
		feature_map[2] <= 0;
...
always @(posedge max_pool_map_clk or negedge rst_n) begin
	if(!rst_n) begin
		max_pool_map <= 0;
	end
	else begin
...
always @(posedge quant2_clk or negedge rst_n) begin
	if(!rst_n) begin
		quant2_map[0] <= 0;
		quant2_map[1] <= 0;
		quant2_map[2] <= 0;
